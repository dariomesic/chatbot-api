...
export default {
                ...
                <div class="container" :class="{ minimized: minimized }">
                    <div class="top" style="padding: 0 0 0 13px;border:unset;background-color:unset">
                        ...
                        <div class="InfoBot">
                            ...
                            <p class="status" style="margin-top:unset">E-Ovrhe</p>
                        </div>
  ...
  `,
                    ...
                    chatContainer.addEventListener('click', async (event) => {
                        ...
                        if (event.target.classList.contains('bot-option')) {
                            if (!(event.target.getAttribute('data-text') == 'PRETRAŽI BAZU ZNANJA' || event.target.getAttribute('data-text') == 'PREFORMULIRAT ĆU PITANJE')) {
                                ...
                                response = await fetch('http://172.20.67.24:8081/getRulesForIntent?intent_id=' + event.target.getAttribute('data-intent-id') + '&system_id=2', {
                                        method: 'GET',
                                    })
                                    .then(response => {
                                        if (!response.ok) {
                            ...
                            await fetch('http://172.20.67.24:8081/updateConversationTmp', {
                                    ...
                                    body: JSON.stringify({ uuid: this.sessionUUID, system_id: "2", intent_id: Number(event.target.getAttribute('data-intent-id')), question: event.target.getAttribute('data-question'), threshold: event.target.getAttribute('data-threshold'), response: response }),
                                })
                                .then((response) => {
                                    if (!response.ok) {
                                        throw new Error(response.error);
    ...
    async mounted() {
        let res = await fetch('http://172.20.67.24:8081/getInitialChat?system_id=2', {
                method: 'GET',
            })
            .then(response => {
                if (!response.ok) {
    ...
    methods: {
        async initializeBot() {
            ...
            let themes = await fetch('http://172.20.67.24:8081/getThemes?system_id=2', {
                method: 'GET',
            })
            .then(response => {
                if (!response.ok) {
                ...
                try {
                    ...
                    if (this.responseApi.response_type === 'Slobodni tekst') {
                        if (this.responseApi.continuation === 'Vrati se na pod-akciju') {
                            let response = await fetch('http://172.20.67.24:8081/goToStep?intent_id=' + this.responseApi.intent_id + '&system_id=2' + '&id=' + this.responseApi.previous_response.id, {
                                    method: 'GET',
                                })
                                .then(response => {
                                    if (!response.ok) {
                        ...
                        if (regEx.test(sanitizedInput)) {
                            ...
                            const errorMessage = "Unijeli ste netočan regularni izraz. Molim Vas pokuÅ¡ajte ponovno.";
        ...
        async handleFeedback(value) {
            try {
                if (value) {
                    await fetch('http://172.20.67.24:8081/thumbsUp?uuid=' + this.sessionUUID + '&system_id=' + "2" + '&intent_id=' + this.intent_id, {
                            method: 'GET',
                        })
                        .then((response) => {
                            if (!response.ok) {
                    ...
                    await fetch('http://172.20.67.24:8081/thumbsDown?uuid=' + this.sessionUUID + '&system_id=' + "2" + '&intent_id=' + this.intent_id, {
                            method: 'GET',
                        })
                        .then((response) => {
                            if (!response.ok) {
